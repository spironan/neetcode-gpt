...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        concat = torch.cat(head_out, dim = 2) # merge into one giant array
        concat = self.out_proj(concat) # apply the final linear projection on it
        return concat.round(decimals=4)
